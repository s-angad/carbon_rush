  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Carbon Sequestration Trend</h3>
              <p className="text-sm text-gray-500">Cumulative verified carbon over time</p>
            </div>
            <div className="flex gap-2">
              {["6M", "1Y", "2Y"].map((range, i) => (
                <button key={range} className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${i === 2 ? "bg-emerald-50 text-emerald-700" : "text-gray-400 hover:text-gray-900 hover:bg-gray-50"}`}>
                  {range}
                </button>
              ))}
            </div>
        ...
        <motion.div
          ...
          className="p-6 rounded-2xl bg-white border border-gray-200 shadow-sm flex flex-col items-center justify-center"
        >
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Carbon Health</h3>
          <p className="text-sm text-gray-500 mb-6">Ecosystem vitality index</p>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          <h3 className="text-lg font-semibold text-gray-900 mb-1">Marketplace Activity</h3>
          <p className="text-sm text-gray-500 mb-4">Monthly trading volume</p>
            ...
            {aiInsights.map((insight, i) => (
              ...
              >
                <div className="flex items-start gap-2">
                  {insight.type === "positive" && <CheckCircle2 className="w-4 h-4 text-emerald-500 mt-0.5 flex-shrink-0" />}
                  {insight.type === "warning" && <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" />}
                  {insight.type === "info" && <Zap className="w-4 h-4 text-sky-500 mt-0.5 flex-shrink-0" />}
                  <div>
            ...
            {recentVerifications.map((v, i) => (
              ...
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${
                  ...
                  v.status === "in_progress" ? "bg-sky-50 text-sky-700" :
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-gray-900 truncate">{v.project}</p>
                ...
                <div className="flex items-center gap-1">
                  {v.status === "completed" && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />}
                  {v.status === "in_progress" && <Clock className="w-3.5 h-3.5 text-sky-500 animate-spin" style={{ animationDuration: "3s" }} />}
                  {v.status === "pending" && <Clock className="w-3.5 h-3.5 text-amber-500" />}
                </div>
              </motion.div>
            ))}
          </div>
