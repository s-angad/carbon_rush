...
const navItems = [
  ...
  { href: "/dashboard/passports", label: "Carbon Passports", icon: Leaf },
  { href: "/dashboard/verification", label: "AI Verification", icon: Brain },
  { href: "/dashboard/fraud", label: "Fraud Detection", icon: ShieldAlert },
  { href: "/dashboard/marketplace", label: "Marketplace", icon: TrendingUp },
  { href: "/dashboard/community", label: "Community", icon: Users },
  { href: "/dashboard/maps", label: "Maps & Monitoring", icon: Map },
  { href: "/dashboard/analytics", label: "Analytics", icon: BarChart3 },
  { href: "/dashboard/blockchain", label: "Blockchain", icon: Blocks },
];

const adminItems = [
  { href: "/dashboard/admin", label: "User Management", icon: UsersRound },
];
      ...
      <aside className={`${sidebarOpen ? "w-64" : "w-[68px]"} hidden lg:flex flex-col bg-white border-r border-gray-200 transition-all duration-200 flex-shrink-0`}>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2.5">
          <div className={`mb-2 ${sidebarOpen ? "px-2" : "px-0"}`}>
            {sidebarOpen && <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-3">Platform</p>}
          </div>
          ...
          {user && sidebarOpen && (
            <div className="flex items-center gap-3 px-2 py-2 mb-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">{user.role === "admin" ? "Administrator" : "User"}</p>
              </div>
            </div>
          )}
          <button
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 border border-gray-100 w-72">
              ...
              <input
                type="text"
                placeholder="Search projects, credits, users..."
