  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: "easeOut" }} className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-lime-500 flex items-center justify-center shadow-sm">
          <TrendingUp className="w-5 h-5 text-white" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Carbon Marketplace</h1>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, ease: "easeOut" }} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="p-4 rounded-2xl bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <motion.div
          ...
          className="lg:col-span-3 p-6 rounded-2xl bg-white border border-gray-200 shadow-sm"
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <div className="flex items-center gap-3">
                <span className="text-2xl font-bold text-gray-900">${currentPrice.toFixed(2)}</span>
                <span className={`flex items-center gap-0.5 text-sm font-medium ${Number(priceChange) >= 0 ? "text-emerald-600" : "text-red-500"}`}>
            ...
            <div className="flex gap-1">
              {["1D", "1W", "1M", "3M", "1Y", "ALL"].map((range) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${timeRange === range ? "bg-emerald-50 text-emerald-700" : "text-gray-400 hover:text-gray-900 hover:bg-gray-50"}`}
                >
                  {range}
                </button>
              ))}
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={priceHistory}>
              ...
              <XAxis dataKey="date" tick={{ fill: "#9CA3AF", fontSize: 10 }} axisLine={false} tickLine={false} interval={14} />
        ...
        >
            ...
            {(["buy", "sell"] as const).map((t) => (
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${tradeType === t
                  ? t === "buy" ? "bg-emerald-600 text-white shadow-sm" : "bg-red-500 text-white shadow-sm"
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block">Quantity (Credits)</label>
              <input
                type="number"
                value={quantity}
                onChange={e => setQuantity(Number(e.target.value))}
                className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-900 text-lg font-medium outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 transition-colors"
              />
              <div className="flex gap-2 mt-2">
                {[100, 500, 1000, 5000].map(amt => (
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all ${quantity === amt ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-gray-50 text-gray-400 border border-gray-100 hover:text-gray-600 hover:border-gray-200"}`}
                  >
                    {amt.toLocaleString()}
                  </button>
                ))}
            ...
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block">Price per Credit</label>
              <div className="px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-900 text-lg font-medium">
            ...
            <div className="p-3 rounded-xl bg-gray-50 border border-gray-100">
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-gray-900 font-bold">${(quantity * currentPrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-gray-400">Platform fee (0.5%)</span>
                <span className="text-gray-500">${(quantity * currentPrice * 0.005).toFixed(2)}</span>
              </div>
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              {buyOrders.map((o, i) => (
                <div key={i} className="relative flex justify-between text-sm py-1.5 px-1">
                  <div className="absolute left-0 top-0 bottom-0 bg-emerald-50 rounded-r" style={{ width: `${(o.quantity / maxOrderQty) * 100}%` }} />
                  <span className="relative text-emerald-600 font-medium">${o.price.toFixed(2)}</span>
                  <span className="relative text-gray-600">{o.quantity.toLocaleString()}</span>
                  <span className="relative text-gray-400 text-xs">${(o.total / 1000).toFixed(1)}K</span>
                </div>
              ))}
            </div>
            {/* Sell Side */}
            <div>
              ...
              {sellOrders.map((o, i) => (
                <div key={i} className="relative flex justify-between text-sm py-1.5 px-1">
                  <div className="absolute right-0 top-0 bottom-0 bg-red-50 rounded-l" style={{ width: `${(o.quantity / maxOrderQty) * 100}%` }} />
                  <span className="relative text-red-500 font-medium">${o.price.toFixed(2)}</span>
                  <span className="relative text-gray-600">{o.quantity.toLocaleString()}</span>
                  <span className="relative text-gray-400 text-xs">${(o.total / 1000).toFixed(1)}K</span>
                </div>
              ))}
              ...
              <thead>
                <tr className="border-b border-gray-100">
                  {["Time", "Type", "Qty", "Price", "Buyer"].map(h => (
                    <th key={h} className="text-left text-xs text-gray-400 uppercase tracking-wider font-medium pb-2">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {recentTrades.map((t, i) => (
                  <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="py-2.5 text-xs text-gray-400">{t.time}</td>
                    <td className="py-2.5">
                      <span className={`inline-flex items-center gap-1 text-xs font-medium ${t.type === "buy" ? "text-emerald-600" : "text-red-500"}`}>
                        {t.type === "buy" ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                        {t.type.toUpperCase()}
                      </span>
                    </td>
                    <td className="py-2.5 text-sm text-gray-900">{t.quantity.toLocaleString()}</td>
                    <td className="py-2.5 text-sm text-gray-900">${t.price.toFixed(2)}</td>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {listings.map((l, i) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.7 + i * 0.05, ease: "easeOut" }}
              className="p-5 rounded-2xl bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow group"
            >
              ...
              <h4 className="text-gray-900 font-semibold mb-1">{l.project}</h4>
              <div className="flex items-center gap-2 mb-3">
                <span className="px-2 py-0.5 rounded text-xs bg-emerald-50 text-emerald-700">{l.type}</span>
                <span className="text-xs text-gray-400">Vintage {l.vintage}</span>
              </div>
              <div className="flex justify-between items-end">
                <div>
                  <div className="text-xs text-gray-400">Available</div>
                  <div className="text-gray-900 font-medium">{l.quantity.toLocaleString()} credits</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-gray-400">Price</div>
                  <div className="text-emerald-600 font-bold text-lg">${l.price.toFixed(2)}</div>
                </div>
              </div>
              <button className="w-full mt-4 py-2.5 rounded-xl bg-emerald-50 text-emerald-700 text-sm font-medium hover:bg-emerald-100 border border-emerald-200 transition-all opacity-0 group-hover:opacity-100">
                Trade Now
              </button>
            </motion.div>
          ))}
